...
int main() {
	...
	double taxas[12] = {0.0112, 0.0092, 0.0117, 0.0092, 0.0112, 0.0107, 0.0107, 0.0114, 0.0097, 0.0100, 0.0092, 0.0089};
	
	// Array para armazenar o grafo (conexões entre os meses com as taxas derentabilidade)
	...
	// Calculando a rentabilidade acumulada e conectando o mês de dezembro anterior até o mês de dezembro seguinte
 	...
 	for (int i = 0; i < 12; i++) {
 		printf("Taxa de %d para %d: %.2f%%\n", i + 1, i + 2, taxas[i] * 100);
 	}
 
 	// Exibindo as arestas acumuladas
 	printf("\nRentabilidade acumulada (de dezembro a cada mês):\n");
 	for (int i = 0; i < 12; i++) {
 		printf("Acumulada do mês %d para %d: %.2f%%\n", i+1, i + 2, grafo[0][i + 1] * 100);
 	}
